...
# --- Rutas de Archivos ---
MODEL_STATE_PATH = os.path.join(get_data_base_path(),'model_state/incremental_xgboost_model.pkl' )
FEATURES_DIR = os.path.join(get_data_base_path(), 'processed' )
LABELS_DIR = os.path.join(get_data_base_path(), 'processed/labels')
LABEL_WEEKLY = os.path.join(get_data_base_path(), 'processed')
CURRENT_PREDICTION_WEEK_FILE = os.path.join(get_data_base_path(), 'model_state/current_prediction_week.txt')
...
class IncrementalXGBoost(BaseEstimator, ClassifierMixin):
    ...
    def train_backlog_until_latest(self):
        while True:
            label_path = os.path.join(LABELS_DIR, f"week_labels_{self.current_week_to_predict}.parquet")
            self.generate_predictions_for_next_week()
            # Si no hay más etiquetas, terminamos el ciclo
    ...
    def generate_predictions_for_next_week(self) -> pd.DataFrame:
        ...
        X_predict['product_id'] = X_predict['product_id'].astype(int)
        X_predict['customer_id'] = X_predict['customer_id'].astype(int)
    ...
    def evaluate_and_detect_drift(self):
        ...
        labels_path = os.path.join(LABELS_DIR, f"week_labels_{week_to_evaluate}.parquet")

        if not os.path.exists(predictions_path) or not os.path.exists(labels_path):
            self._log(logging.INFO, f"Esperando archivo de {week_to_evaluate}.")
            time.sleep(2)
        ...
        eval_df = pd.merge(predictions_df, actual_labels_df, left_index=True, right_index=True, how='left')
